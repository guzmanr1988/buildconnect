-- 109b_admin_employee_full_parity.sql
...
--
-- LIVE-CAPTURE: this migration is SOURCE-OF-TRUTH CAPTURE — the 65 ALTER
-- POLICY statements were applied to llybxug prod via inline Mgmt-API SQL on
-- 2026-06-25 and the canonical .sql file was filed on an unmerged audit-trail
-- branch with a colliding file number (audit-mig-101 collided with the
-- canonical 101_concierge_rep_requests). Renumbered to 109b (matching the
-- original "mig 109 v3" iteration label internally referenced below) to
-- avoid the file-number collision; companion 109_handle_new_user_tighten_
-- block_admin_signup carries the privesc trigger guard that shipped paired
-- with this expansion.
--
-- ───────────────────────────────────────────────────────────────────
-- DRIFT VERIFICATION (against live pg_policies 2026-06-26):
--   Probed all 65 (table, policy) pairs from the body below against
--   llybxugitrbgybplgpsi pg_policies via Mgmt-API. Result:
--     * 65/65 PRESENT — every target policy still exists on live.
--     * 65/65 contain "admin_employee" in qual OR with_check — every
--       expansion is still in place.
--     * 0 missing, 0 stripped, 0 reverted.
--   Verdict: FAITHFUL. Audit body kept verbatim with renumber.
--
--   Sister mig 109 (handle_new_user) showed SHA drift (live sha256
--   10b2f85b != audit claim 1b51dc18) — that mig was rebuilt from live.
--   This mig has no analogous SHA drift on the policy bodies; the
--   semantic-equivalence probe (admin_employee present in qual/with_check
--   for all 65) is the equivalent rail at the pg_policies layer.
--
-- ───────────────────────────────────────────────────────────────────
...
--
-- ───────────────────────────────────────────────────────────────────
-- Historical-fact note (NOT a blocker):
--   mig 107_admin_employee_profiles_select_parity (narrow SELECT-on-
--   profiles single-policy fix shipped 2026-06-25 to unblock the helios
--   assign-rep walker divergence) was SUPERSEDED by this mig's broader
--   65-policy expansion for the specific policy "Admins can view all
--   profiles" — both produced semantically equivalent qual for that one
--   policy (admin OR admin_employee), and both ran idempotently. The
--   sequence (107 narrow → audit-101 broad) is preserved in the canonical
--   migration ordering 107 → 108 → 109 → 109b. No conflict at the
--   pg_policies layer at apply time or now.
--
-- ───────────────────────────────────────────────────────────────────
...
--     (employee → admin self-promotion vector; covered by mig 109 trigger
...
--      the FE real-write path needs it.)
...
--
-- Companion: 109_handle_new_user_tighten_block_admin_signup.sql (privesc
-- trigger guard). Both shipped together as the privesc + parity bundle
-- on 2026-06-25 prod cut.
-- ───────────────────────────────────────────────────────────────────
